...
def hilos():
    ...
    for j in range (numeroHilos):
        tiempo = Thread(target=multiplicacionMatriz, args=(int((n/numeroHilos) * j),int((n/numeroHilos) * (j+1))))#tiempo = es un thread
...
if __name__ == "__main__":
    ...
    f.close()
    print("El proceso de multiplicacion se demoro : " + str(end_time - start_time))
    resultado()
